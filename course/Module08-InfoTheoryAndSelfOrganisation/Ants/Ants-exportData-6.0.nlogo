...
The model now writes 3 files: positionsx.txt, positionsy.txt and headings.txt. These files record (for the first 2000 time steps of a given run) the x positions, y positions and headings respectively for each ant in the simulation at each time step. In each data file, the row index corresponds to time and column index to ant number (as per the usual JIDT data format).
