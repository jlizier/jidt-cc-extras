...
to setup
  ...
  ;; J Lizier -- setting up data output file
  set spins-file (word "spins-" temperature ".txt")
  if file-exists? spins-file [
        file-delete spins-file ;; clear file
  ]
  set sorted-patches sort patches ;; sorts in left-to-right, top-to-bottom order
  reset-ticks
end
...
to capture-data
  ;; To do a one-off:
  ;; file-open "spins.txt"
  ;; ;; Does not select patches in order:
  ;; ask patches [ file-print spin ]
  ;; file-close

  ;; Write the current headings
  file-open spins-file ;; Opening file for writing
  foreach sorted-patches [
    [the-patch] ->
    ask the-patch [
      ifelse spin = 1
        [ file-write 1 ]
        [ file-write 0 ] ;; replacing spin -1 with 0 for file-write
    ]
  ]
...
## EXTENSIONS FOR INFORMATION-THEORETIC ANALYSIS

The model now write a data file: spins-%.2f.txt (with the temperature written to 2 decimal places as %.2f). This file records, after every 1000 steps, the spins (0 or 1) for every dipole in the simulation. This is done 1000 times before halting. In the data file, the row index corresponds to time and column index to dipole number (as per the usual JIDT data format).
...
Adapted by J.T. Lizier under CC BY_NC_SA 3.0 from original code as follows:
